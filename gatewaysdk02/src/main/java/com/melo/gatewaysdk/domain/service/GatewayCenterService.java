package com.melo.gatewaysdk.domain.service;

import cn.hutool.http.HttpUtil;
import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.melo.gatewaysdk.GatewayException;
import com.melo.gatewaysdk.common.Result;
...
public class GatewayCenterService {
    ...
    public void doRegisterApplication(String address, String systemId, String systemName, String systemType, String systemRegistry) {
        ...
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/melo/register/registerApplication", paramMap, 550);
        } catch (Exception e) {
            logger.error("应用服务注册异常，链接资源不可用：{}", address + "/melo/register/registerApplication");
            throw e;
        }
        ...
        if (!"0".equals(result.getCode()) && !"0003".equals(result.getCode()))
    ...
    public void doRegisterApplicationInterface(String address, String systemId, String interfaceId, String interfaceName, String interfaceVersion) {
        ...
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/melo/register/registerApplicationInterface", paramMap, 550);
        } catch (Exception e) {
            logger.error("应用服务接口注册异常，链接资源不可用：{}", address + "/melo/register/registerApplicationInterface");
            throw e;
        }
        Result<Boolean> result = JSON.parseObject(resultStr, new TypeReference<Result<Boolean>>() {
        });
    ...
    public void doRegisterApplicationInterfaceMethod(String address,
        ...
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/melo/register/registerApplicationInterfaceMethod", paramMap, 550);
        } catch (Exception e) {
            logger.error("应用服务接口注册方法异常，链接资源不可用：{}", address + "/melo/register/registerApplicationInterfaceMethod");
            throw e;
        }
        Result<Boolean> result = JSON.parseObject(resultStr, new TypeReference<Result<Boolean>>() {
        });
    ...
    public void doRegisterEvent(String address, String systemId) {
        ...
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/melo/register/registerEvent", paramMap, 550);
        } catch (Exception e) {
            logger.error("应用服务接口事件方法异常，链接资源不可用：{}", address + "/melo/register/registerEvent");
            throw e;
        }
        Result<Boolean> result = JSON.parseObject(resultStr, new TypeReference<Result<Boolean>>() {
        });
